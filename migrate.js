 ...
 */
const DBMigrate = function (driver, log) {
    this.db = driver
 ...
 * @returns {?Strign}
 */
DBMigrate.prototype.getVersion = async function () {
 ...
 * @returns {strign}
 */
DBMigrate.prototype.getTableInfo = async function(tableName) {
...
DBMigrate.prototype.getIndexes = async function(tableName) {
...
DBMigrate.prototype.getColumns = async function(tbl) {
...
DBMigrate.prototype.exportTables = async function() {
    let dump = []
...
DBMigrate.prototype.addColumn = async function(table, column) {
    let spec = new Column(column)
    return await this.exec(`ALTER TABLE \`${table}\` ADD COLUMN ${createColumnDef(spec.name, spec, {})}`)
}

DBMigrate.prototype.renameColumn = async function(table, oldName, newName) {
    let column
...
DBMigrate.prototype.dropColumn = async function(table, columnName) {
    const columns = await this.getColumns(table)
...
DBMigrate.prototype.addIndex = async function(tableName, indexName, columns, unique) {
...
DBMigrate.prototype.dropIndex = async function(tableName, indexName) {
    //tableName not used in sqlite
    return await this.exec(`DROP INDEX IF EXISTS "${indexName}"`)
}

DBMigrate.prototype.addIndexes = async function(tableName, indexes) {
...
DBMigrate.prototype.updateTable = async function(table, columns, mapping) {
...
DBMigrate.prototype.dropTable = async function(tableName){
    return await this.exec(`DROP TABLE \`${tableName}\`;`)
}

DBMigrate.prototype.truncate = async function(tableName) {
...
DBMigrate.prototype.createTable = async function(table, columns, options){
